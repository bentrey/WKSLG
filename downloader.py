...
def wfmu_downloader(includes=None):
    ...
    for n in range(10):
        ...
        playlist_table = ''
        for table in tables:
            artist_in = "artist" in table.lower() or "the stooge" in table.lower()
            track_in = "track" in table.lower()  or "the song" in table.lower()
            if artist_in and track_in:
                playlist_table = '<table'+table.split('</table>')[0]+'</table>'
        files = np.array(os.listdir('library'))
        if playlist_table != '':
            df = pd.read_html(playlist_table)
            if len(df)>0:
                df = df[0].fillna('unknown')
